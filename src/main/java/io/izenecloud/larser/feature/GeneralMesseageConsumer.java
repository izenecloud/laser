...
public class GeneralMesseageConsumer extends LaserMessageConsumer {
	...
	public GeneralMesseageConsumer(String collection, Path output,
		...
		this.fs = fs;
		this.userFeatureMapper = io.izenecloud.conf.Configuration.getInstance()
				.getUserFeatureSerializePath(getCollection());
		this.itemDimension = io.izenecloud.conf.Configuration.getInstance()
				.getItemFeatureDimension(getCollection());
		this.userDimension = io.izenecloud.conf.Configuration.getInstance()
		...
		try {
			List<String> uris = Arrays.asList(io.izenecloud.conf.Configuration
		...
		couchbaseClient = new CouchbaseClient(hosts, io.izenecloud.conf.Configuration
				.getInstance().getCouchbaseBucket(getCollection()),
				io.izenecloud.conf.Configuration.getInstance().getCouchbasePassword(
						getCollection()));

		msgpackClient = new MsgpackClient(io.izenecloud.conf.Configuration
